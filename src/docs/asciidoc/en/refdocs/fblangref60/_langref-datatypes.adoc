...
If the number of bytes is not specified, 255 is used by default.
...
If the number of characters is not specified, 255 is used by default.
|===
...
The `FLOAT` data type defaults to a 32-bit single precision floating-point type with an approximate precision of 7 decimal digits (24 binary digits).
...
|length
|Length in bytes between 1 and 32,765, defaults to 255.
|===
...
|length
|Length in characters, defaults to 255.
...
If no _length_ is specified, it is taken to be 1, or -- for `VARYING` -- 255.

A similar data type is available for the variable-length string type: `++NATIONAL {CHAR | CHARACTER} VARYING++`, with a default length of 255.
...
defaults to 1 for fixed-width (`CHAR`/`BINARY`) and 255 for variable-width (`VARCHAR`/`VARBINARY`).
