...
As SQL statements can contain quotes, it is recommended to use <<langref-commons-qstrings,alternative string literals>> (Q-literals).
...
[#langref-sys-pckg-sql-parse-unqualified-names]
=== Procedure `PARSE_UNQUALIFIED_NAMES`

A selectable procedure that parses a list of unqualified SQL identifiers and returns a row for each identifier.
...
.Input parameters
`RDB$TIME_ZONE_NAME`::
type `CHAR(63)` -- The timezone name
`RDB$FROM_TIMESTAMP`::
type `TIMESTAMP WITH TIME ZONE` -- Start of date range
`RDB$TO_TIMESTAMP`::
type `TIMESTAMP WITH TIME ZONE` -- End of date range
