...
|correlation-name
|Name of the derived table produced by `GENERATE_SERIES` (required!)
...
If not specified, the column does not receive a name, but the column will have the alias `GENERATE_SERIES` when used in a top-level `FROM` clause.
...
* The series stops once the last generated step value exceeds the _limit_ value.
