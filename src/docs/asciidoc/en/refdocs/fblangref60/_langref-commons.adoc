...
A column of the array type cannot be an element in an expression except when used with the <<langref-commons-isnotnull,`IS [NOT] NULL` predicate>>.
...
<<langref-commons-predlike>>, <<langref-commons-predstartwith>>, <<langref-commons-predcontaining>>, <<langref-commons-predsimilarto>>, <<langref-commons-predbetween>>, <<langref-commons-isnotnull>>, <<langref-commons-isnotboolean,`++IS [NOT] {TRUE | FALSE | UNKNOWN}++`>> and <<langref-commons-isnotdistinct>>

Existential predicates::
Predicates used to check the existence of values in a set.
The <<langref-commons-in,`IN` predicate>> can be used both with sets of comma-separated constants and with subqueries that return a single column.
The <<langref-commons-exists>>, <<langref-commons-singular>>, <<langref-commons-quant-all>>, <<langref-commons-quant-anysome>> predicates can be used only with sub-queries.
...
Context variable::
An internally-defined <<langref-contextvars,context variable>>
...
Type cast::
An expression explicitly converting data of one data type to another using the <<langref-scalarfuncs-cast,`CAST` function>> (`CAST (<value> AS <datatype>)`).
...
The internal function <<langref-scalarfuncs-gen-id,`GEN_ID()`>> does the same.

`AT` expression::
Expression to change the time zone of a datetime.

[#langref-commons-constants]
=== Literals (Constants)
...
[#langref-commons-string-constant]
==== String Literals
...
Strings entered this way will be type `BINARY` (a.k.a. `CHAR CHARACTER SET OCTETS`) by default, unless the <<langref-commons-introducer-syntax,_introducer syntax_>> is used to force a string to be interpreted as another character set.

[#langref-binary-literal-syntax]
...
!! For further rules, see <<langref-char-literal-syntax>> !!
----
...
[#langref-commons-introducer-syntax]
...
[#langref-commons-number-constant]
==== Number Literals
...
[#langref-commons-datetime-literal]
==== Datetime Literals
...
Since Firebird 4.0, the use of <<langref-dtyp-tbl-datetimemnemonics,datetime mnemonics>> in datetime literals (e.g. `DATE 'TODAY'`) is no longer allowed.
...
[#langref-commons-datetime-literal-syntax]
...
[#langref-commons-datetime-format-syntax]
...
[#langref-commons-tbl-datetime-literal]
...
.See also
<<langref-scalarfuncs-blob-append,`BLOB_APPEND()`>>

[#langref-commons-arith]
==== Arithmetic Operators
...
.See also
<<langref-commons-othercomppreds,Other Comparison Predicates>>.

[#langref-commons-logical]
==== Logical Operators
...
The `NEXT VALUE FOR` operator is equivalent to the legacy <<langref-scalarfuncs-gen-id,`GEN_ID (..., _increment_)` function>> with _increment_ the increment stored in the metadata of the sequence.
...
The _increment_ of a sequence can be configured with the `INCREMENT` clause of <<langref-ddl-sequence-create,`CREATE SEQUENCE`>> or <<langref-ddl-sequence-alter,`ALTER SEQUENCE`>>.
...
.See also
<<langref-ddl-sequence,`SEQUENCE` (`GENERATOR`)>>, <<langref-scalarfuncs-gen-id,`GEN_ID()`>>
...
All other conditional expressions apply internal functions derived from `CASE` and are described in  <<langref-functions-conditional,Conditional Functions>>.

[#langref-commons-conditional-case]
==== `CASE`
...
A short form of the simple `CASE` construct is the <<langref-scalarfuncs-decode,`DECODE`>> function.

[#langref-commons-conditional-case-srched]
===== Searched `CASE`
...
For the complete list of comparison operators with their variant forms, see <<langref-commons-compar,Comparison Operators>>.
...
The `BETWEEN` predicate in Firebird is asymmetrical -- if the lower bound is not the first argument, the `BETWEEN` predicate will return FALSE.
...
However, if the predicate takes the form of `LIKE 'string%'` (i.e. the only wildcard is `%` at the end), it will be converted to `STARTING WITH 'string'`, which can use an index.
This optimization only works for literal patterns, not for parameters.

So, if you need to search for the beginning of a string, it is recommended to use the <<langref-commons-predstartwith>> predicate instead of the `LIKE` predicate.
====
...
[NOTE]
====
If you need to do a case-insensitive search for something _enclosed inside_ a string (`++LIKE '%Abc%'++`), use of the <<langref-commons-predcontaining>> predicate is recommended.
====
...
.See also
<<langref-commons-predstartwith,`STARTING WITH`>>, <<langref-commons-predcontaining,`CONTAINING`>>, <<langref-commons-predsimilarto,`SIMILAR TO`>>

[#langref-commons-predstartwith]
===== `STARTING WITH`

.Syntax
[listing,subs=+quotes]
----
_expr1_ [NOT] { STARTING | STARTS } [WITH] _expr2_
----

The `STARTING WITH` predicate searches for a string or a string-like type that starts with the characters in its _expr2_ argument.
The case- and accent-sensitivity of `STARTING WITH` depends on the collation of _expr1_.
...
[#langref-commons-predcontaining]
===== `CONTAINING`
...
Feel free to skip it and read the next section, <<langref-commons-buildregex,Building Regular Expressions>>, which uses a bottom-up approach, aimed at the rest of us.
...
[float]
[#langref-commons-regexwildcards]
====== Wildcards
...
[float]
[#langref-commons-regexcharclass]
====== Character Classes
...
[float]
[#langref-commons-regexpredefclass]
...
[float]
[#langref-commons-regexquantifiers]
====== Quantifiers
...
.See also
<<langref-commons-isnotnull>>, <<langref-commons-isnotboolean>>

[#langref-commons-isnotboolean]
===== Boolean `IS [NOT]`
...
The subquery must specify only one column, otherwise the error "`__count of column list and variable list do not match__`" will occur.

Queries using an `IN` predicate with a subquery can be replaced with a similar query using the `EXISTS` predicate.
...
.See also
<<langref-commons-exists,EXISTS>>

[#langref-commons-singular]
==== `SINGULAR`
