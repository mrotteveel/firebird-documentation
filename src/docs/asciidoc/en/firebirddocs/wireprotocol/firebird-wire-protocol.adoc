...
As far as we know, you could use any type, but the following description as a `CHAR(0)` is the most efficient, because you'll send no data at all, and only an `Int32` null-indicator in protocol 10 - 12 or a single bit in the null-bitset in protocol 13 and higher.
